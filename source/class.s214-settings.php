...
class S214_Settings {
	...
	private function prepare_setting_args( $section, array $option ) {
		...
		$args = array_merge( $args,
			isset( $option[ self::CUSTOM_ATTRIBUTES_KEY ] ) ? $option[ self::CUSTOM_ATTRIBUTES_KEY ] : array() );
	...
	public function multicheck_callback( $args ) {
		...
		if ( ! empty( $args['options'] ) ) {
			...
			foreach ( $args['options'] as $key => $option ) {
				...
				$html .= '<input ' . $custom_attributes_html . ' name="' . $this->func . '_settings[' . $args['id'] . '][' . $key . ']" id="' . $this->func . '_settings[' . $args['id'] . '][' . $key . ']" type="checkbox" value="' . $option . '" ' . checked( $option,
						$enabled, false ) . ' />&nbsp;';
	...
	public function radio_callback( $args ) {
		...
		if ( ! empty( $args['options'] ) ) {
			...
			foreach ( $args['options'] as $key => $option ) {
				...
				$html .= '<input ' . $custom_attributes_html . ' name="' . $this->func . '_settings[' . $args['id'] . ']" id="' . $this->func . '_settings[' . $args['id'] . '][' . $key . ']" type="radio" value="' . $key . '" ' . checked( true,
						$checked, false ) . '/>&nbsp;';
				$html .= '<label for="' . $this->func . '_settings[' . $args['id'] . '][' . $key . ']">' . $option . '</label><br />';
			}
	...
	public function sysinfo_callback( $args ) {
		...
		if ( ! isset( $options[ $args['tab'] ] ) || ( isset( $options[ $args['tab'] ] ) && isset( $_GET['tab'] ) && $_GET['tab'] == $options[ $args['tab'] ] ) ) {
			$html = '<textarea ' . $custom_attributes_html . ' readonly="readonly" onclick="this.focus(); this.select()" id="system-info-textarea" name="' . $this->func . '-system-info" title="' . __( 'To copy the system info, click below then press Ctrl + C (PC) or Cmd + C (Mac).',
	...
	public function missing_callback( $args ) {
		printf( __( 'The callback function used for the <strong>%s</strong> setting is missing.', 's214-settings' ),
			$args['id'] );
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script( $this->slug . '-cm-html', $cm_cdn . 'mode/htmlmixed/htmlmixed.js',
			array( 'jquery', $this->slug . '-cm' ), '5.14.2' );
		wp_enqueue_script( $this->slug . '-cm-xml', $cm_cdn . 'mode/xml/xml.js', array( 'jquery', $this->slug . '-cm' ),
			'5.14.2' );
		wp_enqueue_script( $this->slug . '-cm-js', $cm_cdn . 'mode/javascript/javascript.js',
			array( 'jquery', $this->slug . '-cm' ), '5.14.2' );
		wp_enqueue_script( $this->slug . '-cm-css', $cm_cdn . 'mode/css/css.js', array( 'jquery', $this->slug . '-cm' ),
			'5.14.2' );
		wp_enqueue_script( $this->slug . '-cm-php', $cm_cdn . 'mode/php/php.js', array( 'jquery', $this->slug . '-cm' ),
			'5.14.2' );
		wp_enqueue_script( $this->slug . '-cm-clike', $cm_cdn . 'mode/clike/clike.js',
			array( 'jquery', $this->slug . '-cm' ), '5.14.2' );

		wp_enqueue_style( $this->slug . '-s214-settings', $url_path . '/assets/css/admin' . $suffix . '.css', array(),
			$this->version );
		wp_enqueue_script( $this->slug . '-s214-settings', $url_path . '/assets/js/admin' . $suffix . '.js',
			array( 'jquery' ), $this->version );
		wp_localize_script( $this->slug . '-s214-settings', 's214_settings_vars',
			apply_filters( $this->func . 'localize_script', array(
				'func'               => $this->func,
